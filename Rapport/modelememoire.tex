...
\section{Introduction} % SECTION 1
	\subsection{Contexte et motivations}
		...
		être catastrophique.\\
		\\
		Dans le cadre de la RTE, les modèles obtenant les meilleurs taux de réussite, décrits dans la section 3.1, sont non interprétables. 
	...
	\subsection{Tâches}
		...
		la méthode Local Interpretable Model-agnostic Explanations (LIME) \cite{ref2}, grâce à un corpus d'explications que nous avons créé à l'aide de six annotateurs.\\
		\\	
		Nous allons tout d'abord lister nos hypothèses scientifiques, puis énoncer l'état de l'art pour la RTE ainsi que pour l'interprétabilité de prédictions de modèle en général. Nous verrons l'approche de LIME en section 3.3. 
		Nous allons ensuite décrire ce qu'est une "bonne" explication pour savoir sur quoi faut-il se baser pour implémenter une méthode d'interprétabilité en section 4. Puis, nous décrirons les modèles que nous avons implémentés pour la RTE. 
		Ces modèles seront les boîtes noires que nous tenterons d'interpréter. Enfin, nous allons décrire la méthode BAXI puis le cadre expérimental pour donner les différents résultats obtenus. Nous conclurons sur une discussion et sur les différentes perspectives.
...
\section{Hypothèse scientifique} % SECTION 2
	...
	La comparaison est effectuée à l'aide d'une métrique expliquée en section , qui utilise un corpus explicatif contenant 
	les mots les plus intéressants dans la prémisse et dans l'hypothèse. Nous pensons donc que \\
	mesure(BAXI, corpus explicatif) $\underset{RTE}{>}$ mesure(LIME, corpus explicatif).
...
\section{Etat de l'art} % SECTION 3
	...
	\subsection{Dans l'interprétabilité de prédictions de modèles}
		/*PARLER D'INTERPRETABILITE EN GENERAL ET POURQUOI CA NE S'APPLIQUE PAS AU TEXTE*/
		/*PARLER ARTICLE LREC*/
\begin{comment}
...
\section{Définition d'interprétabilité} % SECTION 4 FINI
	...
	\subsection{Qu'est-ce-qu'une "bonne" explication ?}
		...
		Miller résume ce qu'est une bonne explication \cite{ref5}, c'est-à-dire ce que les humains attendent d'une explication. 
		Grâce à cela, nous allons énoncer les types d'explications que BAXI doit suivre.
...
\section{Systèmes implémentés} % SECTION 5 FINI
	...
	\subsection{Système avec mécanisme d'attention}
	...
	\subsection{Résultats des systèmes}	
		...
		\begin{table}[h]
			...
			\hline
			\end{tabular}
			\caption{\bf{Résultats des tests de la RTE pour le fichier de validation.}}
			\end{center}
		\end{table}
		...
		\begin{table}[h]
			...
			\hline
			\end{tabular}
			\caption{\bf{Résultats des tests de la RTE pour le fichier de test.}}
			\end{center}
		\end{table}

\section{Description de l'approche BAXI} % PRESQUE FINI
	...
	De plus, si la probabilité des autres étiquettes a augmenté, alors le mot a d'autant plus d'importance : en le remplacant, l'entrée $x$ a basculé vers une autre étiquette.\\
	\\
	...
	\\
	L'équation 9 pénalise l'aumentation de la probabilité de l'étiquette que l'on veut 
...
\section{Cadre expérimental}
	\subsection{Corpus SNLI et représentation de mots} %FINI
		...
		\begin{table}[h]
			...
			\caption[Echantillon du corpus de développement de SNLI]{\bf{Echantillon de 5 paires du corpus de développement de SNLI présentant à gauche la prémisse, 
				à droite l'hypothèse, et au centre les étiquettes des 5 annotateurs (C pour Contradiction, N pour Neutral, et E pour Entailment) 
		...
		composé de 19 paires prémisse/hypothèse avec leur étiquette associée, appelé Corpus Pour l'Interprétabilité (CPI). 
	...
	\subsection{Mots de remplacement} %PRESQUE FINI
		Pour que BAXI marche bien, il faut remplacer les mots par d'autres mots pertinents. Pour cela, on a annoté 
	...
	\subsection{Corpus d'explications} %PRESQUE FINI
		...
		%\begin{figure}[h] % LE FAIRE PLUS GRAND
		%	\center
		%	\includegraphics[width=16cm, height=16cm, keepaspectratio]{expl_corpus.png}
		%	\caption[Echantillon du corpus d'explications]{\bf{Echantillon du corpus d'explications. La première colonne est l'identifiant de la paire prémisse/hypothèse, 
		%	la deuxième est l'étiquette, la troisième est la prémisse, la quatrième est l'hypothèse, la cinquième contient les mots de la prémisse expliquant l'étiquette, et la dernière colonne contient les mots 
		%	de l'hypothèse expliquant l'étiquette. -1 signifie qu'il n'y a aucun mot important. Chaque numéro à droite d'un mot important correspond à sa position dans la phrase, en commençant par 0.}}
		%\end{figure}	
	...
	\subsection{Paramètres} %FINI
		Tous les \textit{embeddings} sont mis-à-jour par le réseau. La taille des \textit{batches} est de 16. \\
...
\section{Résultats}
	La table ci-dessous montre les taux de réussite des étiquettes prédites pour les trois systèmes décrits en section 5 
	sur l'échantillon du corpus de test de SNLI :\\
	...
	\begin{table}[h]
		...
		\hline
		\end{tabular}
		\caption[Résultats des tests de la RTE pour le corpus de l'échantillon de test.]{\bf{Résultats des taux de réussite des étiquettes prédites pour le CPI.}}
		\end{center}
	\end{table}	
	...
	section <> sur l'échantillon du corpus de test de SNLI et sur le corpus d'explication : \\
	...
	\begin{table}[h]
		...
		\hline
		\multirow{2}{*}{1} & BAXI & 61,70\% & 57,14\% & 51,61\% & 57,61\% \\
			\cline{2-6}
			& LIME & test & test & test & test \\
		\hline
		\multirow{2}{*}{2} & BAXI & 76,60\% & 57,14\% & 54,84\% & 66,30\% \\
			\cline{2-6}
			& LIME & test & test & test & test \\
		\hline
		\multirow{2}{*}{3} & BAXI & 61,70\% & 57,14\% & 51,61\% & 57,61\% \\
			\cline{2-6}
			& LIME & test & test & test & test \\
		\hline
		\end{tabular}
		\caption[Résultats des mesures de BAXI et LIME]{\bf{Résultats des mesures de BAXI et LIME pour le fichier de l'échantillon 
			de test et pour le corpus d'explications.}}
		\end{center}
	\end{table}
