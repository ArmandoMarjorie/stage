...
\section{Etat de l'art}
	...
	\subsection{Dans le domaine de la RTE} 
		...
		Depuis, les plus grands taux de réussite ont atteint les 89,3\% \cite{ref7}.
	\subsection{Dans l'interprétabilité}
		...
		\\
		Nous étudions dans ce rapport la méthode LIME \cite{ref2}, mais nous pouvons également citer la méthode SHapley Additive exPlanations (SHAP) \cite{ref8} : 
...
\section{Réseaux de neurones récurrents}
	\subsection{LSTM et Bi-LSTM}
		...
		Les états cachés sont calculés à chaque instant t comme suit :
		\begin{equation}
		entree : i_t = \sigma(W_i x_t + U_i h_{t-1} + b_i)
		\end{equation}
		\begin{equation}
		oublie : f_t = \sigma(W_f x_t + U_f h_{t-1} + b_f)
		\end{equation}
		\begin{equation}
		etat : u_t = \tanh(W_u x_t + U_u h_{t-1} + b_u)
		\end{equation}
		\begin{equation}
		sortie : o_t = \sigma(W_o x_t + U_o h_{t-1} + b_o)
		\end{equation}
		\begin{equation}
