...
\section{Approches pour la RTE} % SECTION 5
	\paragraph*{Réseaux de neurones récurrent} Nous avons implémenté trois systèmes différents avec la librairie DyNet \cite{ref6}. Les entrées -les mots 
	...
	\subsection{Résultats des systèmes}	
		...
		\begin{table}
			...
			corpus de validation de SNLI, par étiquette et global.}}
			\label{dev}
			\end{center}
		...
		\begin{table}
			...
			\end{tabular}
			\caption{\bf{Résultats des taux de réussite en pourcentage des étiquettes prédites pour le 
			corpus de test de SNLI, par étiquette et global.}}
			\label{test}
			\end{center}
...
\section{Cadre expérimental} % SECTION 7
	...
	\subsection{Corpus explicatif} % FINI
		...
		La figure ~\ref{expl} est un échantillon de ce corpus explicatif.
	...
	\subsection{Corpus d'exemples adversariaux} % FINI		
		...
		expliquer la contradiction, on cherche à créer une contradiction avec les mots de remplacement. Les tables ~\ref{p} et ~\ref{p} 
		...
		La table ~\ref{cpi} présente la distribution des 
...
\section{Résultats} % SECTION 8
	Pour contextualiser, la table ~\ref{acc_cpi} montre les taux de réussite des étiquettes prédites pour les trois systèmes décrits en section 5 
	...
	La table ~\ref{resultat} illustre les différents résultats obtenus pour le taux de réussite des explications des trois systèmes décrits en 
...
\section{Discussion} % SECTION 9 FINI
	...
	\begin{figure}[h]
		...
		\caption{\bf{Exemple d'une explication fournie par BAXI sur le système 2. Les mots surlignés en bleu ont contribué à la neutralité, 
		les mots surlignés en jaune ont contribué à l'inférence, et 
		les mots surlignés en rouge ont contribué à la contradiction. L'étiquette \textit{gold} est "inférence", cependant il n'y a aucune mention d'éclaboussures d'eau dans la prémisse. Selon BAXI, 
...
\section{Conclusion} % SECTION 10 FINI
	...
	retire un ou plusieurs mots sans les remplacer.\\
	Pour se faire, nous avons créé un corpus explicatif comprenant, pour quelques entrées tirées du corpus de test 
