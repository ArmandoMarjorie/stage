...
\def\ResumeFrancais{
    ...
    Le but de la RTE est de déterminer automatiquement si une phrase, appelée l'hypothèse, peut être 
    déduite d'une autre phrase, appelée la prémisse.
    En utilisant des réseaux de neurones, nous pouvons obtenir de bons taux de réussite pour la RTE. Cependant, 
    ...
    Grâce à ce nouveau corpus, nous mesurons le taux d'explications correctes fourni 
...
\section{Introduction} % SECTION 1 FINI
	\subsection{Contexte et motivations}
		Dans la pratique, il y a souvent un compromis entre le taux de réussite et l'interprétabilité d'un modèle. 
		...
		Dans le cadre de la reconnaissance de l'inférence textuelle (Recognizing Textual Entailment : RTE), les modèles obtenant les meilleurs taux de réussite à l'heure actuelle sont non interprétables. 
		Le problème avec l'utilisation de modèles non interprétables est qu'ils fournissent 
		...
		Pourtant, avoir un modèle digne de confiance lui permettrait d'être d'avantage utilisé : en effet, il a été observé, par exemple, que le fait de fournir des 
		...
		Pour qu'un modèle soit digne de confiance, il faut qu'un grand nombre d'explications fournies 
		soit jugé correct par les 
		utilisateurs.
	\subsection{Tâches}
		...
		que la recherche d'information, le raisonnement de bon sens, ou encore les systèmes 
		de question/réponse. 
		Plus spécifiquement, soit une paire de phrase prémisse/hypothèse, la RTE a comme 
		objectif de détecter si la seconde phrase est en contradiction, peut être déduite ou bien est neutre par rapport à la 
		...
		\end{itemize}\\
		\\
		Dans ce travail, nous proposons d'interpréter les prédictions des modèles pour la RTE avec la méthode Best Adversarial eXample for Interpretability (BAXI). 
		...
		Ces modèles sont les boîtes noires que nous tenterons d'interpréter. Enfin, nous allons décrire la méthode BAXI en section 6 puis le cadre expérimental en section 7 pour donner les différents résultats obtenus en section 8. 
...
\section{Hypothèse scientifique} % SECTION 2 PRESQUE FINI
	...
	les mots explicatifs dans la prémisse et dans l'hypothèse (voir section 7.2). 
...
\section{Etat de l'art} % SECTION 3
	\subsection{Dans la RTE}
		...
		grâce à la publication du corpus SNLI (the Stanford Natural Language Inference) décrit dans la section 7.1 \cite{ref}. Cela a permit d'entraîner des réseaux de neurones 
		...
		\\
		Une des méthodes parmi les meilleur taux de réussite est KIM \cite{ref3}, qui utilise un BILSTM, un mécanisme d'attention et des informations extérieures caractérisant la relation sémantique entre 
		un mot de la prémisse et de l'hypothèse grâce à WordNet (voir section 5 pour plus de détails sur les BILSTMs). 
		Le principe est le suivant : les mots de la prémisse et de l'hypothèse sont représentés en les passant dans un BILSTM qui utilise un LSTM \textit{forward} pour 
		...
		KIM définit $p^s$ (respectivement $h^s$), le vecteur de représentation des mots de la prémisse 
		(respectivement de l'hypothèse).
		Une matrice d'alignement est alors construite comme suit :
		\begin{equation}
		e_{ij} = (p_i^s)^T h_j^s + F(r_{ij})
		\end{equation}
		où $p_i^s$ est la représentation du i\up{ème} mot de la prémisse, $h_j^s$ est celle du j\up{ième} mot de l'hypothèse, 
		et $F(r_{ij})$ représente les informations extérieures.
		Avec cette matrice, les vecteurs de contexte $p^c$ et $h^c$ suivants sont construits pour la prémisse et l'hypothèse :
		\begin{equation}
		...
		et vice-versa. 
		KIM compare ensuite la relation entre $p^s$ et $p^c$ pour modéliser l'inférence locale, puis collecte une fois de plus les informations 
		extérieures :

			\begin{flalign}
				&p_i^m = G([p_i^s ; p_i^c; p_i^s - p_i^c; p_i^s $\odot$ p_i^c; \sum_{j=1}^{N}\alpha_{ij} r_{ij}])&
			\end{flalign}
			\begin{flalign}
				&h_j^m = G([h_j^s ; h_j^c; h_j^s - h_j^c; h_j^s $\odot$ h_j^c; \sum_{i=1}^{M}\beta_{ij} r_{ji}])& 
			\end{flalign}
		où $G$ est une fonction non-linéaire pour réduire la dimension, [;] dénote la concaténation, et $\odot$ est le produit 
		de Hadamard.\\
		Enfin, pour déterminer la relation entre la prémisse et l'hypothèse, KIM utilise une couche de composition comme suit :
		\begin{equation}
		p_i^v = \mathop{\mathrm{Composition}(p^m,i) , h_j^v = \mathop{\mathrm{Composition}(h^m,j)
		\end{equation}		
		Puis KIM utilise du \textit{pooling} pondéré, basé sur les informations extérieures, pour obtenir un vecteur de 
		taille fixe :
		\begin{equation}
			p^w = \sum_{i=1}^{M} \frac{exp(H(\sum_{j=1}^{N} \alpha_{ij} r_{ij}))}{\sum_{i=1}^{M} exp(H(\sum_{j=1}^{N} \alpha_{ij} r_{ij}))} p_i^v
		\end{equation}		
		\begin{equation}
			h^w = \sum_{j=1}^{N} \frac{exp(H(\sum_{i=1}^{M} \beta_{ij} r_{ji}))}{\sum_{j=1}^{N} exp(H(\sum_{i=1}^{M} \beta_{ij} r_{ji}))} h_j^v
		\end{equation}		
		où $H$ est un réseau de neurones à propagation avant, avec une couche et la fonction d'activation ReLU. KIM 
		concatène tous les vecteurs de \textit{pooling} (\textit{mean pooling}, \textit{max pooling}, et \textit{pooling} pondéré) 
		dans un vecteur de taille fixe et le donne en entrée à un perceptron multi-couches. 
		KIM obtient 89,1\% de taux de réussite pour le corpus de test de SNLI. 
	\subsection{Dans l'interprétabilité de prédictions de modèles}
		...
		de Molnar dans le chapitre 5.7 \cite{ref5}. 
	...
	\subsection{Description de LIME}
		...
		Tout d'abord, les \textit{features} utilisées et les représentations interprétables des 
		\textit{features} sont à distinguer. Par exemple, 
		les \textit{features} sont les \textit{embeddings} des mots et la représentation 
		interprétable de ces \textit{features} est un vecteur binaire 
		qui indique la présence ou l'absence des mots.\\
		LIME définit une explication par un modèle $g \in G$, où $G$ est la classe des modèles 
		interprétables tels que 
		les modèles linéaires ou les arbres de décisions. Vu que les modèles interprétables n'ont pas 
		...
		On dénote par $f : \mathbb{R}^{d} \rightarrow \mathbb{R}$ le modèle utilisé comme une boîte 
		noire. $f(x)$ est la 
		probabilité que l'entrée $x$ appartienne à une certaine étiquette.\\
		LIME va alors se baser sur la représentation interprétable des données en retirant un ou 
		plusieurs mots au hasard. Cette 
		nouvelle entrée est notée $z$. LIME définit la localité de $x$ avec $\pi_x(z)$ qui est une 
		mesure de proximité entre $z$ et 
		...
		$g$ est infidèle à $f$ dans la localité 
		défini par $\pi_x$. Pour préserver à la fois l'interprétabilité et la fidèlité locale, 
		LIME minimise $\mathcal{L}(f, g, \pi_x)$ 
		avec $\Omega(g)$ assez petit pour être interprétable par les humains. L'explication de 
		LIME est donc la suivante :
		\begin{equation}
		...
		La figure ~\ref{fig_lime} est un exemple illustant le principe de LIME.
		\begin{figure}
			\center
			...
			utilise $f$ pour avoir la probabilité de ces entrées pour l'étiquette $y$, et les pondère par leur proximité 
			par rapport à $x$ (les poids sont représentés par la taille). La droite pointillée est l'explication apprise qui est 
			localement fidèle. \cite{ref2}}}
			\label{fig_lime}
		\end{figure}
		...
		ont utilisé LIME avec un corpus de texte sur l'Athéisme et le Christianisme pour déterminer les mots en faveur de l'un ou l'autre. 
		Ce corpus est problématique car il contient des informations qui ne sont pas généralisables, comme des entêtes de mail. Les auteurs ont alors 
		utilisé un autre corpus en plus basé sur l'Athéisme et le Christianisme, mais n'ayant pas de problèmes quant à son contenu. 
		LIME fournit alors des explications sur chacun de ces corpus, en utilisant un SVM (Support Vector Machine) avec un RBF à noyau (Radial Basis Function kernel). 
		Les auteurs ont ensuite demandé à des utilisateurs ayant des connaissances dans la religion d'inspecter les explications fournies par LIME, pour savoir si : (1) 
		les utilisateurs arrivent à trouver quel classifieur pourrait le mieux généraliser, (2) les utilisateurs pourraient déterminer quelles informations 
		il faut changer dans le corpus pour améliorer le classifieur, et (3) les utilisateurs sont capables d'identifier et de décrire les irrégularités du classifieur.
...
\section{Définition d'interprétabilité} % SECTION 4 FINI
	...
	\subsection{Qu'est-ce-qu'une "bonne" explication ?}
		...
		\paragraph*{Explication compréhensible}
			...
			Si l'explication est complexe, on pourrait la structurer comme le fait la méthode de 
			Vivian et al. décrite en section 3.2 \cite{ref21}.

	

\section{Approches pour la RTE} % SECTION 5
	\paragraph*{Réseaux de neurones récurrent} Nous avons implémenté trois systèmes différents avec la librairie DyNet. Les entrées -les mots 
	de la prémisse et de l'hypothèse- 
	sont représentées par des \textit{embeddings} et les sorties sont les probabilités de chaque étiquette 
	de la RTE. 
	Nous utilisons les Long Short-Term Memory (LSTMs) et les Bidirectionnal Long Short-Term Memory (BILSTMs). Ils permettent de relier des informations vues 
	antérieurement à la tâche 
	courante. On leur donne des informations à chaque instant $t$, créant ainsi une 
	cellule.\\
	\begin{figure}[h]
		\center
		\includegraphics{lstm_schema_.png}
		\caption{\bf{Schéma d'un LSTM contenant trois cellules. Les différentes portes sont 
		représentées dans la cellule du milieu. Les entrées sont les mot $X_{t-1}$, $X_t$ et $X_{t+1}$. 
		Les états cachés calculés sont $h_{t-1}$, $h_t$ et $h_{t+1}$.}}
		\label{lstm}
	\end{figure}\\
	Ce sont des réseaux de neurones récurrent particuliers capable d'apprendre une dépendance 
	...
	Le réseau calcule alors à chaque instant $t$ un état caché $h_t$ comme suit pour chaque mot :\\
	\begin{equation}
	...
	des états cachés et $D_e$ est la dimension des \textit{embeddings}. 
	Un LSTM peut donc générer une représentation 
	cachée d'un mot (en violet dans la figure ~\ref{lstm}), et une représentation cachée d'une phrase qui est l'état caché de la dernière 
	cellule. Un BILSTM est quant à lui un LSTM bidirectionnel qui exploite les mots dans l'ordre naturel 
	et l'ordre inversé.
	
	\paragraph*{Premier système}
		Le premier système passe la prémisse et l'hypothèse en entrée d'un LSTM pour avoir une représentation pour chacune de ces deux phrases.  
		...
		\end{equation}
		où $y$ est un vecteur contenant les probabilités de chaque étiquette, $W$ est 
		la matrice de poids, $LSTM(\textit{prémisse})$ et $LSTM(\textit{hypothèse})$ sont 
		respectivement les représentations issues du LSTM entraînés avec le reste du modèle de 
	...
	\paragraph*{Deuxième système}
		...
		\end{equation}
		où $T$ dénote la transposée. Ce système permet de comparer les phrases, contrairement au système 
		1 qui les concatène.
	\paragraph*{Troisième système}
		Le troisième système est inspiré de la méthode KIM décrite en section 3.1 \cite{ref3}. Nous effectuons les mêmes étapes jusqu'à l'équation 3, mais sans 
		prendre en compte des informations extérieures. 
		Avec les nouvelles représentations pour les mots définies par les vecteurs de contexte, on effectue du \textit{mean-pooling} :
		\begin{equation}
	...
	\subsection{Résultats des systèmes}	
		Les tables ~\ref{dev} et ~\ref{test} montrent les différents résultats des taux de réussite des étiquettes 
		prédites sur le corpus SNLI \cite{ref} (plus de détail sur le corpus SNLI en section 7.1).
		\begin{table}[h]
			\begin{center}
			...
			\hline
			\end{tabular}
			\caption{\bf{Résultats des taux de réussite en pourcentage des étiquettes prédites pour le 
			corpus de validation de SNLI, par étiquette et dans la globalité.}}
			\label{dev}
			\end{center}
		...
		\begin{table}[h]
			...
			\end{tabular}
			\caption{\bf{Résultats des taux de réussite en pourcentage des étiquettes prédites pour le 
			corpus de test de SNLI, par étiquette et dans la globalité.}}
			\label{test}
			\end{center}
			
		\end{table}
		L'état de l'art pour le taux de réussite des étiquettes prédites est à 89,3\% sur le corpus de test 
		de SNLI. Nous avons cependant préféré avoir des systèmes rapide à implémenter avec des performances raisonnables. 
		Nous appliquerons BAXI sur l'ensemble des systèmes publiquement disponibles dans un futur travail.
...
\section{Description de l'approche BAXI} % SECTION 6 FINI
	...
	L'importance d'un mot est une fonction $\mathop{\mathrm{IMP}} : \mathbb{R}^{d_e} \rightarrow \mathbb{R}$ comme suit : 
	
	\begin{equation}
	\mathop{\mathrm{impact}}^{y_{ref}}(m_i) = -p(y_{ref} \: | \: x \leftarrow m_i = m_a) \: + \: p(y_{ref} \: | \: x)
	\end{equation}\\
	\begin{equation}
	\mathop{\mathrm{impact}}^{y_j}(m_i) = \sum_{j=1, j \neq ref}^{|Y|} \: p(y_j \: | \: x \leftarrow m_i = m_a) \: - \: p(y_j \: | \: x)
	\end{equation}\\
	\begin{equation}
	\mathop{\mathrm{IMP}}(m_i) = \; \underset{m_a}{\max} \; (\mathop{\mathrm{impact}}^{y_{ref}}(m_i) \: + \: \mathop{\mathrm{impact}}^{y_j}(m_i))
	\end{equation}
	...
	$y_j$ est une étiquette différente de $y_{ref}$, et $|Y|$ est le nombre d'étiquettes. 
	L'équation 20 pénalise l'augmentation de la probabilité de l'étiquette que l'on veut 
	expliquer lorsque l'on remplace $m_i$ par $m_a$. 
	L'équation 21 encourage l'augmentation des probabilités des autres étiquettes lorsque l'on remplace $m_i$ par $m_a$. 
	Pour calculer l'importance d'un mot, on additionne ces deux impacts (équation 22).
	Cette formule suit notre intuition de base et est similaire à celle proposée par Robnik-Sikonja et Kononenko \cite{ref8}, 
	mais également à celle proposée par Fong et Vedaldi \cite{ref25} décrit en section 3.2. Nous avons rajouté la prise en compte de l'impact sur  
	...
	\begin{figure}[h]
		...
		les mots surlignés en rouge ont contribué à la contradiction. La probabilité de chaque étiquette 
		est spécifiée entre parenthèse.}}
	\end{figure}
		
\section{Cadre expérimental} % SECTION 7
	\subsection{Corpus SNLI et représentation de mots} %FINI
		Nous utilisons les corpus SNLI annotés pour la tâche de la RTE, composés d'un fichier d'entraînement, de validation et de test, 
		contenant respectivement 550 152, 10 000 et 10 000 exemples composés de paires prémisse/hypothèse avec l'étiquette 
		correspondante. 
		...
		La table ~\ref{snli} est un échantillon du corpus SNLI. 
		\begin{table}[h]
		\leftskip -2cm
			{
				\includegraphics[width=16cm, height=10cm, keepaspectratio]{table_SNLI.png}
			}
	...
	\subsection{Corpus explicatif} % FINI
		...
		de la prémisse et les mots de l'hypothèse qui conduisent à l'étiquette associée.
		Nous avons récolté leurs réponses pour en faire un corpus d'explications de références, au format CSV. 
		La table ~\ref{expl} est un échantillon de ce corpus explicatif.
		...
		\begin{table}
			...
			\caption{\bf{Echantillon du corpus explicatif. Chaque ligne correspond à une paire. Le nombre à droite de chaque mot explicatif est sa position dans sa phrase correspondante : certains mots pouvent 
	...
	\subsection{Corpus d'exemples adversariaux} % FINI		
		...
		trois fichiers -un pour chaque étiquette- contenant 19 paires se trouvant dans le corpus explicatif. 
		Nous n'avons pas encore pu annoter les 43 exemples (la totalité du corpus explicatif), ce sera fait dans un futur travail. Dans ces 
		...
		se trouve être l'étiquette \textit{gold} de la paire prémisse/hypothèse, on cherche alors à remplacer les mots des 
		phrases par des mots pouvant baisser fortement la probabilité de l'étiquette. Par exemple, 
		si l'étiquette \textit{gold} est inférence, on va chercher à basculer vers la neutralité ou la contradiction pour faire 
		baisser l'inférence et ainsi créer un exemple adversarial.
		A l'inverse, si l'étiquette que l'on veut expliquer n'est pas l'étiquette \textit{gold}, on cherche alors à remplacer 
		les mots de la phrase par des mots pouvant augmenter la probabilité de l'étiquette expliquée (et par la même occasion, 
		faire baisser la probabilité des autres étiquettes). Par exemple, si l'étiquette \textit{gold} est neutre et que l'on veut 
		expliquer la contradiction, on cherche à créer une contradiction avec les mots de remplacement. Les tables ~\ref{h}  
		présentent un exemple pour la prémisse et l'hypothèse.
		\begin{table}
			\begin{center}
			\begin{tabular}{|c|c|}
			\hline
		...
		La table 6 présente la distribution des 
		étiquettes présentes dans les 19 exemples que nous évaluons.
		\begin{table}
			\begin{center}
			\begin{tabular}{|c|c|c|}
			\hline
			...
			\hline
			\end{tabular}
			\caption{\bf{Distribution en pourcentage des étiquettes présentes dans le petit corpus d'évaluation contenant 19 exemples issus du corpus 
			de test de SNLI.}}
			\end{center}
			\label{cpi}
		\end{table}
	...
	\subsection{Paramètres} %FINI
		...
		L'entraînement des systèmes 1, 2 et 3 est réalisé avec l'optimiseur Adam et \textit{Negative softmax log likelihood} est utilisé pour calculer le \textit{loss} pendant 20 époques.
		Pour l'utilisation de LIME, on a fixé le nombre de voisin de l'entrée à 500.
		
\section{Résultats} % SECTION 8
	Pour contextualiser, la table 7 montre les taux de réussite des étiquettes prédites pour les trois systèmes décrits en section 5 
	sur le petit corpus d'évaluation contenant 19 exemples.
	\begin{table}[h]
		\begin{center}
		\begin{tabular}{|c|c c c|c|}
		\hline
		...
		\hline
		\end{tabular}
		\caption{\bf{Résultats des taux de réussite des étiquettes prédites sur le petit corpus d'évaluation contenant 19 exemples, pour 
		chaque étiquette et dans la globalité.}}
		\end{center}
		\label{acc_cpi}
	\end{table}	
	
	La table 8 illustre les différents résultats obtenus pour le taux de réussite des explications des trois systèmes décrits en 
	section 5 sur le petit corpus d'évaluation, à l'aide du corpus explicatif. 
	Le taux de réussite des explications correctes est mesuré par le nombre de mots explicatifs correctement trouvés sur le nombre de mots explicatifs total pour 
	le petit corpus d'évaluation. Les vrais mots explicatifs se trouvent dans le corpus explicatif. 
	\begin{table}
		\begin{center}
		...
		\hline
		\end{tabular}
		\caption{\bf{Résultats des taux d'explications correctes en pourcentage de BAXI et LIME sur le petit corpus d'évaluation contenant 19 exemples, pour 
		chaque étiquette et dans la globalité.}}
		\end{center}
		\label{resultat}
	\end{table}
	BAXI est meilleur que LIME mais il a du mal à expliquer la contradiction. Peut être faudrait-il plus d'exemples et de meilleurs mots remplaçants pour améliorer 
	cela. Nous pouvons penser que BAXI est meilleur que LIME car contrairement à LIME qui crée des exemples qui n'ont pas forcément de sens, BAXI crée des exemples
    adversariaux qui pourraient être de vraies entrées. Les graphiques \ref{graph_baxi} représentent les systèmes 1, 2 et 3 dans l'espace de représentation de bons sytèmes : on estime 
    qu'un graphique comme ceux-ci doit être fourni pour déterminer si un modèle est bon ou non.
  
  	\begin{figure}
		\center
		\includegraphics[width=10cm, height=7cm, keepaspectratio]{lime_graph.png}
		\label{lime_graph}
	\end{figure}     
  	\begin{figure}
		\center
		\includegraphics[width=10cm, height=7cm, keepaspectratio]{baxi_graph.png}
		\caption{\bf{Représentation des systèmes 1, 2 et 3 (les carrés bleus) dans l'espace de représentation de bons modèles en utilisant BAXI (figure du bas) 
		et LIME (figure du haut). Un bon modèle doit se rapprocher le 
		plus possible du coin en haut à droite (bonne performance + bonnes explications).}}
		\label{graph_baxi}
	\end{figure}  
...
\section{Discussion} % SECTION 9 FINI
	...
	%pour être certain qu'il retrouve toujours ce genre de biais, ce qui est dans nos projets futurs. 
	...
	De plus, il arrive que certains éléments de l'image présentée ne se trouvent pas dans la prémisse, ce qui engendre des étiquettes difficiles à prédire. La figure ~\ref{ex} en est un exemple.
	...
	\begin{figure}
		...
		\caption{\bf{Exemple d'une explication fournie par BAXI sur le système 2. Les mots surlignés en bleu ont contribué à la neutralité, 
		les mots surlignés en jaune ont contribué à l'inférence, et 
		les mots surlignés en rouge ont contribué à la contradiction. Les probabilités des étiquettes sont entre parenthèses. L'étiquette \textit{gold} est "inférence", cependant il n'y a aucune mention d'éclaboussures d'eau dans la prémisse. Selon BAXI, 
	\textit{"splashing"} a donc contribué à la neutralité. Il a également contribué à l'inférence, ce qui n'est pas surprenant puisqu'il est en présence de mots tels que \textit{"river"} et \textit{"water"}.}}
		\label{ex}
	\end{figure}
	...
	pour mesurer le taux d'explications correctement donnés. Nous pensons qu'en plus du taux de réussite des étiquettes prédites, il faut prendre en compte le taux de réussite des explications pour savoir si 
	un modèle est bon ou non. 

\section{Conclusion} % SECTION 10 FINI
	Pour conclure, nous avons présenté la méthode BAXI pour interpréter les prédictions de modèles 
	dans le cadre de la RTE. BAXI donne les mots dans la prémisse et dans 
	l'hypothèse ayant contribués à l'étiquette que l'on veut "expliquer", en donnant 
	une valeur d'importance à chaque mot. Pour cela, BAXI remplace un 
	seul mot dans une entrée. Le mot retiré est remplacé par un mot de sorte à créer un exemple 
	adversarial. On regarde alors l'impact de ce changement sur les probabilités de l'étiquette pour 
	pouvoir donner une valeur d'importance au mot retiré : si la probabilité de l'étiquette que l'on veut 
	expliquer a augmenté, alors le mot n'avait pas d'importance pour l'étiquette considérée. De plus, 
	si la probabilité des autres étiquettes a baissé, alors le mot n'avait pas d'importance non plus. 
	Nous avons comparé BAXI à la méthode LIME qui explique n'importe quel prédiction de modèle en 
	apprenant localement un modèle interprétable dans le voisinage de l'entrée. Contrairement à BAXI, 
	LIME créé des nouvelles entrées qui n'ont pas forcément de sens pour créer un voisinage, car LIME 
	retire un ou plusieurs mots sans les remplacer. D'après les résultats, nous expliquons le fait que BAXI soit meilleur que LIME 
	sur ce fait. 
	Pour l'évaluation du taux d'explications correctes, nous avons créé un corpus explicatif comprenant, pour quelques entrées tirées du corpus de test 
	de SNLI, les mots expliquant l'étiquette associée à l'entrée. Nous comptons alors le taux de bons 
	mots explicatifs trouvés par les deux méthodes, uniquement pour les étiquettes \textit{gold}. 
	Les résultats montrent que BAXI trouve de meilleurs explications. Pour nos futurs travaux, nous 
	améliorerons BAXI en lui permettant de remplacer plusieurs mots au lieu d'un seul, ce qui résoudrait 
	le problème des expressions polylexicales. Nous allons également réaliser plus d'exemples pour 
	tester la qualité des explications, car le corpus d'évaluation est actuellement très petit (19 exemple) car l'annotation de mots remplaçants est une tâche très longue. Ce serait intéressant 
	de rechercher des méthodes moins chères pour créer des corpus d'exemples adversariaux. 
...
\section{Remerciements} %SECTION 11 FINI
	...
	Enfin, je remercie les annotateurs du corpus explicatif, Thomas Delfino, Franck Dary, Simone Fuscone, Sébastien Ratel, et Jérémy Auguste
	 pour m'avoir accordé un peu de temps lors de la création de ce corpus, ainsi que José-Luis Vilchis Medina pour son aide de manière générale.
